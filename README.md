...
- **Sessions sharing one directory cannot be told apart.** Several Codex
  ...
  all report the same working directory, so nothing distinguishes them. Clicking
  their rows is guaranteed to reach *different* terminals — the sessions are
  spread over the candidate windows, and repeated clicks walk the rest — but not
  guaranteed to reach each session's own. Neither Ghostty nor Codex exposes a
  per-window identity that would settle it; `WindowIdentity` documents what was
  measured.
