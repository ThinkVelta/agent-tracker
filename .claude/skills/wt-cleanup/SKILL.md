...
Then list candidate local branches, **excluding** the default branch, the current branch, and
any branch that has a worktree:

```bash
git branch --format='%(refname:short)'
```

Filter that list in your head (or with grep -v) against `$DEFAULT_BRANCH`, `$CURRENT_BRANCH`,
and the worktree branches above. **Do not hardcode `main|master`** — many repos use `develop`,
`trunk`, or custom default branches, and the user may currently be checked out on a branch that
isn't the default.
...
- **Was its remote deleted?** (common after merging a PR on GitHub)
  ...
  ```
  The `[gone]` marker means the branch once tracked a remote that no longer exists — a strong
  signal the PR was merged and the remote branch deleted.
...
Stale worktrees (no commits in 7+ days, clean):
  ...
  fix/icon-flicker       .claude/worktrees/fix-icon-flicker        8d ago
...
Branches with deleted remote (PR likely merged on GitHub):
  feat/session-store     remote gone — safe to delete locally
  fix/hook-timeout       remote gone — safe to delete locally
...
- **Skip unmerged orphans** — list them but don't touch (inform the user of `git branch -D` if
  they want to force)

Ask which of these the user wants to proceed with. Don't auto-execute anything — the user
confirms first.
...
# Close stale worktrees. Stale ⇒ verified clean in Step 1, so a direct removal
# is safe here — there are no per-worktree services to tear down in this repo.
...
If a removal refuses because the worktree turned dirty since the scan, route it through
`/wt-close <branch>` instead — that skill walks the user through the uncommitted changes.
...
**This is a housekeeping tool, not a destructive one.** The default posture is conservative —
show what could be cleaned up, let the user decide. The `--dry-run` flag makes it completely
safe to explore.

**Merged branches are safe to delete.** Their commits live in the target branch. Unmerged
branches might contain work the user forgot about — flag them but don't delete without explicit
confirmation.

**Use `git worktree remove`, not `rm -rf`.** And use `git branch -d` (safe delete), not `-D`. If
something refuses to delete, that's useful information — surface it.
