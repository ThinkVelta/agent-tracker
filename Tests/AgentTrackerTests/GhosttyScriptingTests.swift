...
final class GhosttyScriptingTests {
    ...
    ///
    /// Compared as a *subset relation*, not equality: this reads a live Ghostty,
    /// and a window opened or closed between the two calls is the user going
    /// about their day, not the specifier misbehaving. Observed failing exactly
    /// that way. What a half-accepted specifier would produce — surfaces
    /// vanishing from a second read, or ids that are not stable identities — is
    /// still caught, because one read has to remain a subset of the other.
